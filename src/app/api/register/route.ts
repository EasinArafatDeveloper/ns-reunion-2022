...
export async function POST(req: Request) {
  try {
    ...
    // 3. Setup Nodemailer Transporter and Send Email (Non-blocking with strict 3-second timeout)
    if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
      try {
        ...
        console.log(`Attempting to send registration email to ${data.email} with 3s timeout...`);
        ...
        console.error('Nodemailer Email Error (Gracefully handled):', emailError.message || emailError);
      }
    } else {
      console.warn('WARNING: EMAIL_USER or EMAIL_PASS environment variables are missing. Registration email skipped.');
    }
